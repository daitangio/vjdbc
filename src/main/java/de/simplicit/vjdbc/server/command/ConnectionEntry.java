...
class ConnectionEntry implements ConnectionContext {
    ...
    private static Logger _logger = Logger.getLogger(ConnectionEntry.class.getName());
        ...
        try {
            if (!_connection.isClosed()) {
                ...
                if (_logger.isLoggable(Level.FINE)) {
                    _logger.fine("Closed connection " + _uid);
                }
            }

            traceConnectionStatistics();
        } catch (SQLException e) {
            _logger.severe("Exception during closing connection");
            e.printStackTrace();
        }
    }
    ...
    public synchronized Object executeCommand(Long uid, Command cmd, CallingContext ctx) throws SQLException {
        try {
            ...
            if (uid != null) {
                ...
                if (target != null) {
                    if (_logger.isLoggable(Level.FINE)) {
                        //GG Uselsess _logger.fine("Target for UID " + uid + " found");

                        for (Entry<Long, JdbcObjectHolder> e : _jdbcObjects.entrySet()) {
                            _logger.fine("" + e.getKey() + ")" + e.getValue());
                        }

                    }
                    // Execute the command on the target object
                    try {
                        _logger.fine("Executing: " + cmd);
                        ...
                        if (_connectionConfiguration.isIgnoreSQLFeatureNotSupportedExceptions()) {
                            _logger.warning("Eating SQLFeatureNotSupportedException and using a null return value");
                        } else {
                    ...
                    if (uidResult != null) {
                        ...
                        if (_logger.isLoggable(Level.FINE)) {
                            _logger.fine("Registered " + result.getClass().getName() + " with UID " + uidResult);
                        }
                        ...
                        if (result != null
                                && VJdbcConfiguration.singleton().isUseCustomResultSetHandling()) {
                            if (result instanceof ResultSet) {
                                ...
                                if (cmd instanceof ResultSetProducerCommand) {
                                    ...
                                    _logger.fine("Command " + cmd.toString() + " doesn't implement "
                                ...
                                if (_logger.isLoggable(Level.FINE)) {
                                    _logger.fine("... returned " + result);
                                }
                            }
                        }
                    }
                } else {
                    _logger.warning("JDBC-Object for UID " + uid + " and command " + cmd + " is null !");
                }
            } else {
            ...
            _logger.fine(cmd + " RESULT: " + result);
            return result;
        } finally {
            _active = false;
            _lastAccessTimestamp = System.currentTimeMillis();
    ...
    private Object handleResultSet(ResultSet result, boolean forwardOnly, CallingContext ctx) throws SQLException {
        ...
        if (_logger.isLoggable(Level.FINE)) {
            _logger.fine("Registered ResultSet with UID " + uid.getUID());
        }
    ...
    private void dumpClientInfoProperties() {
        if (_logger.isLoggable(Level.INFO) && !_clientInfo.isEmpty()) {
